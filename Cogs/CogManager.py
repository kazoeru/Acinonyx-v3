...
class CogManager(commands.Cog):
    ...
    async def imports(self, ctx, *, extension = None):
        """Menghubungkan semua extension atau extension tertentu kedalam bot (Owner-only)"""
        if extension == None:
        ...
        for ext in os.listdir("Cogs"):
            ...
            if ext[:-3].lower() == extension.lower():
                ...
                if not len(import_list):
                    await ctx.send("Extension yang kamu minta tidak mengimport pada extension local.")
                else:
                    await ctx.send("Import:\n\n{}".format(", ".join(import_list)))
                return
        await cxt.send("Tidak dapat menemukan extension yang kamu minta...")


    @commands.command(pass_context=True)
    async def extension(self, ctx, *, extension = None):
        """Melihat extension tertentu pada bot ini."""
        if extension == None:
            # run the extensions command
            await ctx.invoke(self.extensions)
            return
        ...
        for e in self.bot.extensions:
            ...
            if len(cog_list):
                ...
                help_embed.add_field(name="No Cogs", value="└─ 0 command", inline=True)
            await ctx.send(embed=help_embed)
            return
        await ctx.send("Tidak dapat menemukan extension yang kamu minta.")
    ...
    async def reload(self, ctx, *, extension = None):
        """Reloads semua extension atau extension tertentu (owner-only)."""
        # Only allow owner
        ...
        if extension == None:
            message = await ctx.send("Memuat ulang semua extensions...")
        ...
        message = await ctx.send("Mencoba memuat ulang extension...".format(extension.replace("`","").replace("\\","")))
        result = self._load_extension(extension)
        
        if result[1] == 0:
            await message.edit(content="Aku tidak dapat menamukan extension yang kamu cari.")
        else:
            e_string = "extension" if result[1] == 1 else "extensions"
            await message.edit(content="Sukses memuat ulang {}/{} {} telah terhubung dengan bot!".format(result[0], result[1], e_string))
