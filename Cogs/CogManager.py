...
class CogManager(commands.Cog):
    ...
    async def _send_embed(self, ctx, embed, pm = False):
        # Melihat semua extension yang terdapat pada bot ini
    ...
    async def imports(self, ctx, *, extension = None):
        """Melihat extension yang terhubung dengan extension pendukung (Owner-only)"""
        if extension == None:
        ...
        for ext in os.listdir("Cogs"):
            ...
            if ext[:-3].lower() == extension.lower():
                ...
                if not len(import_list):
                    msg = "Extension yang kamu minta tidak mengimport pada extension local."
                    ...
                    msg = "File *{}.py* terhubung dengan:\n\n{}".format(extension, ", ".join(import_list))
                    em = discord.Embed(color = 0XFF8C00, description = msg)
                    em.set_footer(text = "{}#{}".format(ctx.author.name, ctx.author.discriminator),
                                  icon_url = "{}".format(ctx.author.avatar_url))
                    await ctx.send(embed = em)
                return
        msg = "┐(￣ヘ￣;)┌\nAku tidak dapat menemukan file extension yang kamu minta..."
        em = discord.Embed(color = 0XFF8C00, description = msg)
        em.set_footer(text = "{}#{}".format(ctx.author.name, ctx.author.discriminator),
                      icon_url = "{}".format(ctx.author.avatar_url))
        await ctx.send(embed = em)
    ...
    async def extension(self, ctx, *, extension = None):
        """Melihat extension kategori pada bot ini."""
        if extension == None:
            # run the extensions command
            em = discord.Embed(color = 0XFF8C00,
                           description = "Melihat jumlah command pada extension tertentu.\n"
                                         "**Panduan**\n"
                                         "`{}extension [extension]`\n\n"
                                         "***Contoh***\n"
                                         "`{}extension Humor`\n\n"
                                         "*Catatan:*\n"
                                         "*Untuk melihat semua list extension ketik `{}extensions`*"
                                         .format(ctx.prefix,
                                                 ctx.prefix,
                                                 ctx.prefix))
        ...
        for e in self.bot.extensions:
            ...
            if type(ctx.author) is discord.Member:
                help_embed = discord.Embed(color=0XFF8C00)
            else:
            ...
            if len(cog_list):
                ...
                if total_commands == 1:
                    ...
                    comm = "└─ {:,} {}".format(total_commands, comm)
                help_embed.add_field(name=", ".join(cog_list), value=comm, inline=True)
                help_embed.set_footer(text = "ketik `{}help {}` untuk melihat command pada extension ini\n{}#{}"
                                      .format(ctx.prefix,
                                              ", ".join(cog_list),
                                              ctx.author.name,
                ...
                help_embed.add_field(name="Tidak ada Command", value="Extension ini hanya sebagai pendukung bot", inline=True)
            await ctx.send(embed=help_embed)
            return
        msg = "┐(￣ヘ￣;)┌\nAku tidak menemukan extension yang kamu minta"
        em = discord.Embed(color = 0XFF8C00, description = msg)
    ...
    async def extensions(self, ctx):
        """Melihat seluruh extension cog pada bot ini."""
        # Build the embed

        confirm = ['ya']
        msgConfirm = "Command ini akan mengirimkan 3 halaman embed dalam pesan DM kamu.\nKetik `ya` untuk melanjutkan"
        ...
        if page_total > 1:
            help_embed.title = "Extension (Halaman {:,} dari {:,})".format(page_count, page_total)
        else:
            help_embed.title = "Extension"
        for embed in key_list:
            ...
            if len(help_embed.fields) >= 25:
                if page_total == page_count:
                    if len(ext_list) == 1:
                        help_embed.set_footer(text="Total 1 Extension")
                    else:
                        help_embed.set_footer(text="Total {} Extension".format(len(ext_list)))
                await self._send_embed(ctx, help_embed, to_pm)
                help_embed.clear_fields()
                page_count += 1
                if page_total > 1:
                    help_embed.title = "Extensions (Halaman {:,} dari {:,})".format(page_count, page_total)
        
        if len(help_embed.fields):
            if len(ext_list) == 1:
                help_embed.set_footer(text="1 Extension Total")
            else:
                help_embed.set_footer(text="{} Extension Total".format(len(ext_list)))
            await self._send_embed(ctx, help_embed, to_pm)
        
    
    @commands.command(pass_context=True)
    async def reload(self, ctx, *, extension = None):
        """Reload semua extension atau extension tertentu (owner-only)."""
        ...
        if extension == None:
            msg = "Memuat ulang semua extensions..."
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}#{}".format(ctx.author.name, ctx.author.discriminator), icon_url = "{}".format(ctx.author.avatar_url))
            message = await ctx.send(embed = em)
            result = self._load_extension()
            res_str = "*{}* dari *{}* extension berhasil dimuat ulang!".format(result[0], result[1])
            em = discord.Embed(color = 0XFF8C00, description = res_str)
            em.set_footer(text = "{}#{}".format(ctx.author.name, ctx.author.discriminator), icon_url = "{}".format(ctx.author.avatar_url))
            await message.edit(embed = em)
            return
        msg = "Mencoba memuat ulang extension...".format(extension.replace("`","").replace("\\",""))
        em = discord.Embed(color = 0XFF8C00, description = msg)
        em.set_footer(text = "{}#{}".format(ctx.author.name, ctx.author.discriminator), icon_url = "{}".format(ctx.author.avatar_url))
        message = await ctx.send(embed = em)
        ...
        if result[1] == 0:
            msg = "Aku tidak dapat menamukan extension yang kamu cari."
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}#{}".format(ctx.author.name, ctx.author.discriminator), icon_url = "{}".format(ctx.author.avatar_url))
            await message.edit(embed = em)
        else:
            e_string = "extension" if result[1] == 1 else "extensions"
            msg = "Sukses memuat ulang {}/{} {} telah terhubung dengan bot!".format(result[0], result[1], e_string)
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}#{}".format(ctx.author.name, ctx.author.discriminator), icon_url = "{}".format(ctx.author.avatar_url))
            await message.edit(embed = em)
    ...
    async def update(self, ctx):
        """Update backup semua script bot ke github (owner-only)."""
        isOwner = self.settings.isOwner(ctx.author)
        if isOwner == None:
            msg = 'Belum ada owner.'
            await ctx.channel.send(msg)
