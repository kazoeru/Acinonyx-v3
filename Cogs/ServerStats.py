...
class ServerStats(commands.Cog):
    ...
    async def topservers(self, ctx):
        """Menampilkan top server dihitung dengan member."""
        our_list = []
        for guild in self.bot.guilds:
            our_list.append(
                {
    ...
    async def joinedatpos(self, ctx, *, position):
        ...
        contoh: `acx!joinedatpos 1`"""
        try:
            position = int(position)-1
            assert -1<position<len(ctx.guild.members) 
        except:
            return await ctx.send("Posisi harus dari 1 sampai {:,}".format(len(ctx.guild.members)))
    ...
    async def firstservers(self, ctx):
        """Melihat posisi server saat bot ini bergabung."""
        our_list = []
        # offset = self.settings.getGlobalUserStat(ctx.author, "TimeZone",self.settings.getGlobalUserStat(ctx.author,"UTCOffset",None))
        for guild in self.bot.guilds:
            bot = guild.me
    ...
    async def messages(self, ctx):
        """Melihat sudah berapa banyak aku mengirim pesan di server ini"""
        ...
        if messages == None:
            messages = 0
        if messages == 1:
            await ctx.channel.send('Sejauh ini aku sudah mengirim *{:,} pesan dalam server ini*!*'.format(messages))
        else:
            await ctx.channel.send('Sejauh ini aku sudah mengirim *{:,} pesan dalam server ini*!*'.format(messages))

    @commands.command()
    async def allmessages(self, ctx):
        """Melihat sudah berapa banyak aku mengirim pesan di semua server"""
        messages = 0
        for guild in self.bot.guilds:
        ...
        messages -= 1
        if messages == 1:
            await ctx.channel.send('Sejauh ini aku sudah mengirim *{:,} pesan di semua server!*'.format(messages))
        else:
            await ctx.channel.send('Sejauh ini aku sudah mengirim *{:,} pesan di semua server!*'.format(messages))
