...
class BotAdmin(commands.Cog):
    ...
    async def ignore(self, ctx, *, member = None):
        ...
        if type(member) is str:
            ...
            if not member:
                msg = 'Tidak dapat menemukan *{}*...'.format(memberName)
    ...
    async def kick_ban(self, ctx, members_and_reason = None, command_name = "kick"):
        ...
        for index,item in enumerate(args):
            if self.mention_re.search(item): # Check if it's a mention
                ...
                if member.id == self.bot.user.id: return await ctx.send("Jangan pak, aku nggk mau keluar dari server ini!! :cry:...".format(command_name))
                if member.id == ctx.author.id: return await ctx.send("Kok ngekick diri sendiri?...".format(command_name))
                if Utils.is_bot_admin(ctx,member): return await ctx.send("Kamu tidak dapat menggunakan {} pada admin lain!".format(command_name))
                ...
                break
        if not len(targets): return await ctx.send("Tidak dapat menemukan member!")
        if len(targets) > 5: return await ctx.send("Kamu hanya dapat menggunakan {} sampai dengan 5 member dalam 1 command!".format(command_name))
        if not len(reason): return await ctx.send("Alasan kick dibutuhkan!")
        ...
        confirmation_message = await Message.EmbedText(title="Konfirmasi {}".format(command_name.capitalize()),description=msg,color=ctx.author).send(ctx)
        ...
        if not confirmation_user.content == confirmation_code: return await ctx.send("Kode verifikasi salah.\n{} member dibatalkan!".format(command_name.capitalize()))
        ...
        await Message.EmbedText(title="Hasil {}".format(command_name.capitalize()),description=msg).edit(ctx,message)
