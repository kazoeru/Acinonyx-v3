...
class Discogs(commands.Cog):
    ...
    async def discogs(self, ctx, *, search = None):
        """Melakukan pencarian discogs.com untuk album, artist, dll.
        Kamu dapat mencari secara langsung untuk judul lagu.
        Tapi kamu juga dapat mencari dengan format berikut:

        track=Judul lagi
        album=Judul album
        artist=Nama artis/penyanyi
        type=ketik untuk mencari, bisa salah sari dari: semua format, artis, release, atau master
        
        Contoh:
        acx!discogs artist=my first story album=The Plugless"""
