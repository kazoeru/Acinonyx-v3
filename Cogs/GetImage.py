...
async def get(ctx, url, title = None, ua : str = 'CorpNewt DeepThoughtBot', **kwargs):
	...
	if not afile:
		return await Message.Embed(description="Aku tidak dapat mendapatkan gambar...", color = 0XFF8C00).edit(ctx, message)
	message = await Message.Embed(description="Uploading...", color = 0XFF8C00).edit(ctx, message)
