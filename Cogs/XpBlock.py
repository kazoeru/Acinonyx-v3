...
class XpBlock(commands.Cog):
    ...
    async def xpblock(self, ctx, *, user_or_role : str = None):
        ...
        em = discord.Embed(color = 0XFF8C00,description = "> Menambahkan user atau role kedalam xp block list\n> \n"
                                                          ...
                                                          "> `{}xpblock [user/role]`"
                                                          .format(ctx.prefix))
        em.set_author(name = "xpblock command", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
    ...
    async def xpunblockall(self, ctx):
        ...
        if len(xparray) == 1:
            await ctx.send("*1* user/role telah diunblock dari system xp.")
        else:
            await ctx.send("*{}* user/role telah diunblock dari system xp.".format(len(xparray)))
    ...
    async def xpunblock(self, ctx, *, user_or_role : str = None):
        ...
        em = discord.Embed(color = 0XFF8C00,description = "> Menghapus user atau role dari list xp block\n> \n"
                                                          ...
                                                          "> `{}xpunblock [user/role]`"
                                                          .format(ctx.prefix))
        em.set_author(name = "xpunblock command", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        em.set_footer(text = "Saat mengetik command, tanda [] tidak usah digunakan\nRequest by : {}".format(ctx.author.name), icon_url = "{}".format(ctx.author.avatar_url))
        usage = em
    ...
    async def listxpblock(self, ctx):
        ...
        if not len(promoArray):
            ...
            em.set_footer(text = "Saat mengetik command, tanda [] tidak usah digunakan\nRequest by : {}".format(ctx.author.name), icon_url = "{}".format(ctx.author.avatar_url))
            roleText = em
            await ctx.channel.send(embed = roleText)
            return
