...
class Bot(commands.Cog):
    ...
    async def botinfo(self, ctx):
        ...
        # Build the embed
        fields = [
            {"name":"Total Member","value":member_count,"inline":True},
            {"name":"Total Server","value":guild_count,"inline":True},
            {"name":"Total Command","value":command_count + " ({})".format(cog_count),"inline":True},
            {"name":"Tanggal Bot Dibuat","value":created_at,"inline":True},
            {"name":"Pemilik","value":owners,"inline":True},
    ...
    async def nickname(self, ctx, *, name : str = None):
        ...
        if name == None:
            msg =  "Merubah nama ku dalam server mu\n"
            msg += "**Panduan:**\n"
            msg += "*`{} nickname [name]`*".format(ctx.prefix)
            em = discord.Embed(color = 0XFF8C00, description = msg)
    ...
    async def avatar(self, ctx, filename = None):
        ...
        if not os.path.isfile(filename):
            if not os.path.isfile('./{}'.format(filename)):
                msg = '*{}* tidak dapat menemukan gambar dalam direktori.'.format(filename)
                em = discord.Embed(color = 0XFF8C00, description = msg)
                em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
        ...
        if not ext:
            ...
            msg = '*{}* File bukan format gambar, silahkan coba lagi!'.format(filename)
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
            return await status.edit(embed = em)
    ...
    @commands.command(pass_context=True, aliases = ["donasi"])
    async def donate(self, ctx):
        """Kamu suka dengan bot ini?
        Yuuk bantu bot ini agar tetap hidup."""
        msg =  "Hallo~\n"
        msg += "Kamu suka dengan bot ini?\n"
        msg += "Yuuk bantu bot ini agar tetap hidup, berapapun jumlah bantuan mu akan sangat berarti.\n"
        msg += "Klik link donasi dibawah ini.\n"
        msg += "**[Donasi](https://saweria.co/NvStar)**"
        em = discord.Embed(color = 0XFF8C00, description = msg)
        em.set_image(url = "https://cdn.discordapp.com/attachments/744831859543769199/794400433530601472/unknown.png")
        em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
        await ctx.send(embed = em)
    ...
    async def servers(self, ctx):
        ...
        msg = "Aku berada pada *{}* server{}!".format(len(self.bot.guilds),"" if len(self.bot.guilds) == 1 else "s")
        em = discord.Embed(color = 0XFF8C00, description = msg)
        em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url)) 
        await ctx.send(embed = em)
    ...
    async def status(self, ctx, status = None):
        ...
        if status == None:
            ...
            msg = "Aku sedang dalam status *{}!*".format(botmem.status)
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url)) 
            return await ctx.send(embed = em)
        ...
        if status == "1" or status.lower() == "online":
            ...
            msg = "Option yang kamu masukan salah."
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url)) 
            return await ctx.send(embed = em)
        ...
        await self._update_status()
        msg = "Status berhasil dirubah ke *{}!*".format(stat_name)
        em = discord.Embed(color = 0XFF8C00, description = msg)
        em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url)) 
        await ctx.send(embed = em)
    ...
    async def set_status(self, ctx, status, status_name="Playing", status_type=0, status_url=None):
        ...
        await self._update_status()
        msg = 'Status {} berhasil dirubah ke **{}**{}!'.format(status_name,Utils.suppressed(ctx,status)," at `{}`".format(status_url) if status_url else "")
        em = discord.Embed(color = 0XFF8C00, description = msg)
        em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url)) 
        await message.edit(embed = em)
