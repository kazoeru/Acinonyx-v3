...
todo_list = """``` 
• Harus Update python ke versi 3.9
    - Backup semua data bot (selesai)
    - Ubah semua DataClasses bot dari semua extension
    - Buat inspect function docstrings __slots__ untuk class AudioClip

• Perbaikan command Osu! dengan parameter 
  [mode] dan [player] yang dipisahkan.
• Command spotify downloader harus bisa multiprocess.
• Harus bisa bikin bot ini jadi 2 bahasa 
  `inggris & indonesia`.
• ChatterBot harus bisa menggunakan 2 bahasa 
  "dengan membuat 2 file.brn (brain) dari AIML,
  yang di ambil dari metode
...
class Bot(commands.Cog):
    ...
    async def botinfo(self, ctx):
        ...
        message = await Message.EmbedText(title="Mendapatkan informasi...", color=0XFF8C00).send(ctx)
        ...
        fields = [
            ...
            {"name":"Commands","value":command_count + " ({})".format(cog_count),"inline":True},
    ...
    async def hostinfo(self, ctx):
        ...
        CPU = ProgressBar.center('{}% of {} {}'.format(cpuUsage, cpuThred, threadString), 'CPU') + '\n'
        #CPU += ProgressBar.makeBar(int(round(cpuUsage)))
        RAM = ProgressBar.center('{} ({}%) of {}GB used'.format(memUsedGB, memPerc, memTotalGB), 'RAM') + '\n\n'
        #RAM += ProgressBar.makeBar(int(round(memPerc)))
        ...
        em.set_author(name = "Informasi host", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        ...
        if embed == None:
            em = discord.Embed(color = 0XFF8C00,
                               description = "> **Panduan penggunaan**\n"
                                             ...
                                             .format(ctx.prefix,
                                                     ctx.prefix))
            em.set_author(name = "Embed (JSON)", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
    ...
    async def speedtest(self, ctx):
        """Test kecepatan speed bot (owner only)."""
        if not await Utils.is_owner_reply(ctx): return
    ...
    async def adminunlim(self, ctx, *, yes_no : str = None):
        """Mengatur apakah admin memiliki xp unlimited? (bot-admin only)."""
    ...
    async def avatar(self, ctx, filename = None):
        ...
        if filename is None and not len(ctx.message.attachments):
            m = await ctx.send("Menghapus avatar...")
            try:
                await self.bot.user.edit(avatar=None)
            except discord.errors.HTTPException as e:
                return await m.edit(content="┐(￣ヘ￣;)┌\nSepertinya aku tidak bisa melakukannya sekarang\nSilahkan coba lagi nanti!")
            return await m.edit(content='Avatar dihapus!')
        ...
        isTemp = False
        status = await ctx.send('Memeriksa URL dan mengunduh jika valid...')
        ...
        if not os.path.isfile(filename):
            if not os.path.isfile('./{}'.format(filename)):
                return await status.edit(content='*{}* tidak dapat menemukan gambar dalam direktori.'.format(filename))
            else:
                # Local file name
        ...
        if not ext:
            ...
            return await status.edit(content='*{}* File bukan format gambar, silahkan coba lagi!'.format(filename))

        wasConverted = False
        # Is an image PIL understands
        if not ext.lower == "png":
            # Not a PNG - let's convert
            await status.edit(content='Convert ke png...')
        ...
        if isTemp:
            ...
            if wasConverted:
                os.remove(filename)
        await status.edit(content='Avatar telah diset!')

    # Needs rewrite!
    @commands.command(pass_context=True)
    async def reboot(self, ctx, force = None):
        """Reboot bot (owner only)."""
        if not await Utils.is_owner_reply(ctx): return
    ...
    async def servers(self, ctx):
        """Hey lihat aku ada di berapa server!"""
        await ctx.send("Aku berada pada *{}* server{}!".format(len(self.bot.guilds),"" if len(self.bot.guilds) == 1 else "s"))
    ...
    async def pres(self, ctx, playing_type="0", status_type="online", game=None, url=None):
        ...
        if playing_type.lower() in [ "0", "play", "playing" ]:
            ...
            if url == None or not any("twitch.tv" in x.lower() for x in Utils.get_urls(url)):
                ...
                return await ctx.send("Kamu membutuhkan url twitch.tv untuk memasang status streaming!")
    ...
    async def status(self, ctx, status = None):
        """Merubah online status bot (owner-only).
        Options:
        1. Online
        2. Idle
        3. DnD
        ...
        if status == None:
            ...
            return await ctx.send("Aku sedang dalam status *{}!*".format(botmem.status))
        ...
        if status == "1" or status.lower() == "online":
            ...
            return await ctx.send("Option yang kamu masukan salah.")

        self.settings.setGlobalStat("Status",stat_string)
        await self._update_status()
        await ctx.send("Status berhasil dirubah ke *{}!*".format(stat_name))
    ...
    async def set_status(self, ctx, status, status_name="Playing", status_type=0, status_url=None):
        ...
        if status == status_url == None:
            ...
            await self._update_status()
            return await message.edit(content='status {} dihapus!'.format(status_name))

        if status_type == 1:
            if not status:
                return await ctx.send("Kamu membutuhkan url jika mode streaming!")
            if not any("twitch.tv" in x.lower() for x in Utils.get_urls(ctx)):
                return await ctx.send("Kamu membutuhkan url dari twitch.tv untuk streaming!")
        ...
        msg = 'Merubah status {},\nmenjadi status *{}*...'.format(status_name.lower(), status)
        message = await ctx.send(Utils.suppressed(ctx,msg))

        await self._update_status()
        await message.edit(content='Status {} berhasil dirubah ke **{}**{}!'.format(status_name,Utils.suppressed(ctx,status)," at `{}`".format(status_url) if status_url else ""))
    ...
    async def setbotparts(self, ctx, *, parts : str = None):
        """Setting bot parts.
        bisa dengan URL, atau format text."""
        if not await Utils.is_owner_reply(ctx): return
    ...
    async def cloc(self, ctx):
        """Menghitung jumlah total baris dalam repo yang terinstall.
        
        Metode ini diambil dan telah di edit dari:
        ...
        # Set up our output
        msg = 'Owner ku yang kesepian telah meluangkan waktu untuk menulis dan menghidupkan jiwa ku:\n```\n'
        padTo = 0
        ...
        for idx, val in enumerate(code_count):
            ...
            msg += numString + " " + lineWord + extensions[idx] + "\n"
