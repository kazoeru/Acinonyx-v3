...
class Morse(commands.Cog):
    ...
    async def morse(self, ctx, *, content = None):
        """Convert ascii menjadi morse code.
        Ascii a-z and 0-9."""
        ...
        if content == None:
            em = discord.Embed(color = 0XFF8C00, description =  "> **Panduan**\n"
                                                                "> `{}morse [message]`"
                                                                .format(ctx.prefix))
            em.set_author(name = "Morse", url = "https://acinonyxesports.com/", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
            em.set_footer(text = f"Request By : {ctx.author.name}", icon_url = f"{ctx.author.avatar_url}")
            await ctx.send(embed=em)
            return
    ...
    async def unmorse(self, ctx, *, content = None):
        """Convert morse code menjadi ascii."""
