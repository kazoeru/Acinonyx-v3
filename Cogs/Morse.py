...
class Morse(commands.Cog):
    ...
    async def morse(self, ctx, *, content = None):
        """Convert string (text) menjadi morse code.
        Ascii a-z and 0-9."""

        if content == None:
            em = discord.Embed(color = 0XFF8C00, description =  "Convert string (text) menjadi morse code\n\n"
                                                                "**Panduan**\n"
                                                                "`{}morse [message]`"
                                                                .format(ctx.prefix))
            em.set_footer(text = f"{ctx.author}", icon_url = f"{ctx.author.avatar_url}")
        ...
        if not len(morse_list):
            ...
            msg = "┐(￣ヘ￣;)┌\nTerjadi kesalahan, coba lagi nanti yaa."
            em = discord.Embed(color = 0XFF8C00, description = msg)
        ...
        msg = "```\n" + msg + "```"
        if len(msg) > 1000:
            msg = "Kebanyakan!!!\nterlalu banyak bilangan `1` dan `0` yang harus ku berikan.\nCobalah memasukan text yang lebih singkat...."
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author))
            await ctx.send(embed = em)
            return
    ...
    async def unmorse(self, ctx, *, content = None):
        """Convert morse code menjadi string (text)."""

        if content == None:
            em = discord.Embed(color = 0XFF8C00, description =  "Convert morse menjadi string(text)\n\n"
                                                                "**Panduan**\n"
                                                                "`{}unmorse [message]`\n\n"
                                                                "***Note:***\n"
                                                                "*Untuk melihat list morse ketik*\n"
                                                                "*`{}morsetable`*"
        ...
        em = discord.Embed(color = 0XFF8C00, description =  "**Input**\n```{}```\n"
                                                            ...
                                                            .format(content,
                                                                    msg))
        em.set_author(name = "Morse table list", url = "https://acinonyxesports.com/", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
