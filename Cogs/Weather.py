...
class Weather(commands.Cog):
    ...
    async def tconvert(self, ctx, *, temp = None, from_type = None, to_type = None):
        ...
        em = discord.Embed(color = 0XFF8C00, description = "> Convert suhu Fahrenheit, Celsius, dan Kelvin.\n> \n"
                                                           ...
                                                           "> `K`-Kelvin"
                                                           .format(ctx.prefix))
        em.set_author(name = "tconvert Command", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        ...
        if not(f) or not(t):
            # No valid types
            await ctx.send("Format type temperatur yang tersedia: {}".format(", ".join(types)))
            return
        if f == t:
            # Same in as out
            await ctx.send("Tidak ada perubahan saat Konversi {} ---> {}.".format(f, t))
            return
    ...
    def get_weather_text(self, r = {}, show_current = True):
        ...
        desc += "{}\n\nSuhu tertinggi {} °F ({} °C) - Suhu terendah {} °F ({} °C)\n\n".format(
            condition,
            maxf, maxc,
            minf, minc
        )
    ...
    async def weather(self, ctx, *, city_name = None):
        """Cek cuaca."""
        if city_name == None:
            em = discord.Embed(color = 0XFF8C00, description = "> Cek cuaca\n> \n"
                                                               "> **Panduan**\n"
                                                               "> `{}weather [nama_kota]`"
                                                               .format(ctx.prefix))
            em.set_author(name = "Weather command", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
            em.set_thumbnail(url = "{}".format(ctx.message.guild.icon_url))
            em.set_footer(text = "Saat mengetik command, tanda [] tidak usah digunakan\nRequest by : {}".format(ctx.author.name), icon_url = "{}".format(ctx.author.avatar_url))
            await ctx.send(embed=em)
            return
        ...
        await Message.EmbedText(
            ...
            description=desc,
            color=0XFF8C00,
            footer="Powered by OpenWeatherMap\nRequest By: {}".format(ctx.author.name)
        ).send(ctx)
