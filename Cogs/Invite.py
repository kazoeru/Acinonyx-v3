...
class Invite(commands.Cog):
    ...
    async def requestjoin(self, ctx, invite_url = None):
        ...
        if temp:
            ...
            return await ctx.send("Berlaku hingga {}.".format(ReadableTime.getReadableTimeBetween(time.time(),temp[1])))
    ...
    async def approvejoin(self, ctx, server_id = None):
        ...
        if request:
            await request[3].send("{}, request mu untuk ku join ke server {} telah disetujui hingga {}.\nKamu bisa invite aku dengan link ini:\n<{}>".format(
                request[0].mention,
        ...
        await ctx.send("Aku telah disetujui untuk bergabung dengan server {}\nHingga {}.".format(
            server_id,
            ReadableTime.getReadableTimeBetween(0,self.approval_time)))
    ...
    async def revokejoin(self, ctx, server_id = None):
        ...
        self.temp_allowed.remove(temp)
        return await ctx.send("Persetujuan untuk bergabung dalam server id {} telah dibatalkan.".format(server_id))
    ...
    async def canjoin(self, ctx, *, yes_no = None):
        ...
        if yes_no == None:
            ...
            yes_no = False
            msg = "{} {} *disabled*.".format(setting_name,"masih" if not current else "sekarang")
        else:
