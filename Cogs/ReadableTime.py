...
def getReadableTimeBetween(first, last, reverse=False):
    ...
    if years > 0:
        msg += "1 tahun, " if years == 1 else "{:,} tahun, ".format(years)
    if months > 0:
        msg += "1 bulan, " if months == 1 else "{:,} bulan, ".format(months)
    if weeks > 0:
        msg += "1 minggu, " if weeks == 1 else "{:,} minggu, ".format(weeks)
    if days > 0:
        msg += "1 hari, " if days == 1 else "{:,} hari, ".format(days)
    if hours > 0:
        msg += "1 jam, " if hours == 1 else "{:,} jam, ".format(hours)
    if minutes > 0:
        msg += "1 menit, " if minutes == 1 else "{:,} menit, ".format(minutes)
    if seconds > 0:
        msg += "1 detik, " if seconds == 1 else "{:,} detik, ".format(seconds)
