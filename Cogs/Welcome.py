...
class Welcome(commands.Cog):
    ...
    async def setwelcome(self, ctx, *, message = None):
        """Mengatur welcome message untuk server mu (owner/admin-server only). 
        ...
        if message == None:
            ...
            return await ctx.send('Welcome message telah dihapus!')

        self.settings.setServerStat(ctx.guild, "Welcome", message)
        await ctx.send('Welcome message telah di update!\n\nIni tampilan untuk welcome message dalam server ini:')
    ...
    async def testwelcome(self, ctx, *, member = None):
        """Test welcome message diserver mu (owner/admin-server only)."""
    ...
    async def setgoodbye(self, ctx, *, message = None):
        ...
        if message == None:
            ...
            return await ctx.send('Goodbye message telah dihapus!')

        self.settings.setServerStat(ctx.guild, "Goodbye", message)

        await ctx.send('Goodbye message telah diupdate!\n\nIni tampilan untuk goodbye message dalam server ini:')
    ...
    async def testgoodbye(self, ctx, *, member = None):
        """Test goodbye message diserver mu (owner/admin-server only)."""
        if not await Utils.is_bot_admin_reply(ctx): return

        if member == None:
            member = ctx.message.set_author
        if type(member) is str:
            memberName = member
            member = DisplayName.memberForName(memberName, ctx.guild)
            if not member:
    ...
    async def setwelcomechannel(self, ctx, *, channel : discord.TextChannel = None):
        """Mengatur channel untuk welcome dan goodbye messages (owner/admin-server only)."""
        if not await Utils.is_bot_admin_reply(ctx): return

        if channel == None:
            self.settings.setServerStat(ctx.guild, "WelcomeChannel", "")
            if self._getDefault(ctx.guild):
                msg = 'Welcome dan goodbye messages akan ditampilkan dalam default channel (**{}**).'.format(self._getDefault(ctx.guild).mention)
            else:
        ...
        msg = 'Welcome dan goodbye messages akan ditampilkan dalam channel **{}**.'.format(channel.mention)
        await ctx.send(msg)
