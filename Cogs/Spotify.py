                    ...
                    if user == ctx.author:
                        ...
                        if emoji == '<a:acx_mp3:744868331382767617>':
                            ...
                            if checkServer > 1:
                                ...
                                await ctx.send(file = discord.File(f"/Temp/{ctx.author.id}/{artist_name} - {track_name}.mp3"))

                                botKernel_DeleteFile = subprocess.Popen(["rm", "-rf", f"/Temp/{ctx.author.id}/{artist_name} - {track_name}.mp3"], stdout = subprocess.PIPE).communicate()[0]
                                
                                await dld.delete()
                                await ctx.send(f"{ctx.author.mention} :arrow_up:")

                            checkFile = os.path.getsize(f"/Temp/{ctx.author.id}/{artist_name} - {track_name}.mp3")
                            if checkFile > 8000000:
                                ...
                                computer_path = '/Temp/{}/{} - {}.mp3'.format(ctx.author.id, artist_name, track_name)
                                ...
                                botKernel_DeleteFile = subprocess.Popen(["rm", "-rf", '/Temp/{}/{} - {}.mp3'.format(ctx.author.id, artist_name, track_name)], stdout = subprocess.PIPE).communicate()[0]
                            ...
                            await ctx.send(file = discord.File('/Temp/{}/{} - {}.mp3'.format(ctx.author.id, artist_name, track_name)))
