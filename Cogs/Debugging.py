...
class Debugging(commands.Cog):
    ...
    async def logging(self, ctx):
        """Melihat apakah logging bot sedang menyala atau tidak dalam server mu? (admin only)."""
        if not await Utils.is_bot_admin_reply(ctx): return
    ...
    async def logenable(self, ctx, *, options = None):
        """Mengaktifkan log, dengan pilihan option yang dipisahkan oleh koma (admin only).
    ...
    async def logdisable(self, ctx, *, options = None):
        """Menonaktifkan log, dengan pilihan option yang dipisahkan oleh koma(admin only).
    ...
    async def setlogchannel(self, ctx, *, channel : discord.TextChannel = None):
        """Mengatur channel untuk output log (admin only)."""
        if not await Utils.is_bot_admin_reply(ctx): return
