import asyncio, discord, os, textwrap, time, random
...
class Debugging(commands.Cog):
    ...
    async def on_xp(self, to_user, from_user, amount):
        ...
        if type(to_user) is discord.Role:
            msg = "🌟 {}#{} ({}) memberikan {} xp ke role {}.".format(from_user.name, from_user.discriminator, from_user.id, amount, to_user.name)
        else:
            msg = "🌟 {}#{} ({}) memberikan {} xp ke {}#{} ({}).".format(from_user.name, from_user.discriminator, from_user.id, amount, to_user.name, to_user.discriminator, to_user.id)
    ...
    async def on_member_ban(self, guild, member):
        ...
        msg = '🚫 {}#{} ({}) telah di banned dari server {}.'.format(member.name, member.discriminator, member.id, Utils.suppressed(guild, guild.name))
    ...
    async def on_member_unban(self, guild, member):
        ...
        pfpurl = member.avatar_url if len(member.avatar_url) else member.default_avatar_url
        msg = '🔵 {}#{} ({}) telah di unbanned dari server {}.'.format(member.name, member.discriminator, member.id, Utils.suppressed(guild, guild.name))
    ...
    async def on_mute(self, member, guild, cooldown, muted_by):
        ...
        # A memeber was muted
        pfpurl = member.avatar_url if len(member.avatar_url) else member.default_avatar_url
        msg = "🔇 {}#{} ({}) telah di mute.".format(member.name, member.discriminator, member.id)
        message = "Mute by {}.\nMute {}.".format(
    ...
    async def on_unmute(self, member, guild):
        ...
        # A memeber was muted
        pfpurl = member.avatar_url if len(member.avatar_url) else member.default_avatar_url
        msg = "🔊 {}#{} ({}) telah di unmute.".format(member.name, member.discriminator, member.id)
        await self._logEvent(guild, "", title=msg, color=discord.Color.green(),thumbnail=pfpurl)
    ...
    async def on_member_join(self, member):
        ...
        if not self.shouldLog('user.join', guild):
            return
        # A new member joined
        msg = '👐 {}#{} ({}) bergabung ke server {}.'.format(member.name, member.discriminator, member.id, Utils.suppressed(guild, guild.name))
        log_msg = "Tanggal Akun dibuat: {}".format("Unknown" if member.created_at == None else member.created_at.strftime("%b %d %Y - %I:%M %p") + " UTC")
    ...
    async def on_member_remove(self, member):
        ...
        if not self.shouldLog('user.leave', guild):
            return
        # A member left
        msg = '👋 {}#{} ({}) meninggalkan server {}.'.format(member.name, member.discriminator, member.id, Utils.suppressed(guild, guild.name))
    ...
    async def on_member_update(self, before, after):
        ...
        if not before.status == after.status and self.shouldLog('user.status', server):
            ...
            await self._logEvent(server, msg, title="👤 {}#{} ({}) Update.".format(before.name, before.discriminator, before.id), color=discord.Color.gold(), thumbnail=pfpurl)
    ...
    async def on_message(self, message):
        ...
        if self.shouldLog('message.send', message.guild):
            # A message was sent
            title = '📧 {}#{} ({}), channel #{}, chat:'.format(message.author.name, message.author.discriminator, message.author.id, message.channel.name)
            msg = message.content
            ...
            if "discord.gg/" in mes or "discordapp.com/invite/" in mes:
                ...
                return
            title = '🎫 {}#{} ({}), dalam channel #{}, mengirim invite:'.format(message.author.name, message.author.discriminator, message.author.id, message.channel.name)
    ...
    async def on_message_edit(self, before, after):
        ...
        title = '✏️ {}#{} ({}), channel #{}, edit message:'.format(before.author.name, before.author.discriminator, before.author.id, before.channel.name)
        msg = before.content
    ...
    async def on_message_delete(self, message):
        ...
        title = '❌ {}#{} ({}), channel #{}, delete message:'.format(message.author.name, message.author.discriminator, message.author.id, message.channel.name)
        msg = message.content
        if len(message.attachments):
            msg += "\n\n--- Attachments ---\n\n"
            for a in message.attachments:
    ...
    async def clean(self, ctx, messages = None, *, chan : discord.TextChannel = None):
        ...
        try:
            ...
            return await ctx.send("Aku membutuhkan jumlah pesan yang akan dihapus!")
        # Make sure we're actually trying to clean something
        if messages < 1:
            return await ctx.send("Aku tidak dapat menghapus pesan kurang dari 1 pesan!")
        ...
        try:
            await ctx.author.send('*{}* message{} telah dihapus dari *#{}* di server *{}!*'.format(counter, "" if counter == 1 else "s", chan.name, Utils.suppressed(ctx, ctx.guild.name)))
            # PM the file
        ...
        if self.shouldLog('message.delete', ctx.guild):
            # We're logging
            logmess = '{}#{} menggunakan clean di server #{}'.format(ctx.author.name, ctx.author.discriminator, chan.name)
            pfpurl = ctx.author.avatar_url if len(ctx.author.avatar_url) else ctx.author.default_avatar_url
            await self._logEvent(ctx.guild, "{:,} message{} dihapus.".format(counter, "" if counter == 1 else "s"), title=logmess, filename=filename, thumbnail=pfpurl)
    ...
    #                                                            "`{}logpreset [off/quiet/normal/verbose]`"
    #                                                            .format(ctx.prefix))
    #         em.set_author(name = f"Logpreset", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
    ...
    async def logging(self, ctx):
        """Melihat apakah logging bot sedang menyala atau tidak dalam server mu? (bot-admin only)."""
        if not await Utils.is_bot_admin_reply(ctx): return
        ...
        if logChannel:
            ...
            if channel:
                ...
                des = '> Status Logging *Menyala*\n> Dalam channel *{}*.\n> \n> Logging: {}'.format(channel.mention, logText)
                em = discord.Embed(color = 0XFF8C00, description = des)
                em.set_author(name = f"Status log {ctx.guild.name}", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
                em.set_footer(text = "Request by : {}".format(ctx.author.name), icon_url = "{}".format(ctx.author.avatar_url))
                return await ctx.send(embed = em)
        embed = discord.Embed(color = 0XFF8C00, description = "> Status Logging saat ini *Tidak aktif*\n> \n"
                                                              "> Kamu dapat melihat panduan log dengan mengetik\n"
                                                              "> `{}help logenable`"
                                                              .format(ctx.prefix))
        embed.set_author(name = "Status log {}".format(ctx.guild.name), url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        embed.set_footer(text = "Request by : {}".format(ctx.author.name), icon_url = "{}".format(ctx.author.avatar_url))
        await ctx.send(embed = embed)
        
        
    @commands.command(pass_context=True)
    async def logenable(self, ctx, *, options = None):
        """Mengaktifkan log, dengan pilihan option yang dipisahkan oleh koma (bot-admin only).
        
        Option yang tersedia:
        invite.create, invite.delete, invite.send, user.ban, user.unban, user.mute, user.unmute, user.join, user.leave, user.status,
        user.game.name, user.game.url, user.game.type, user.avatar,
        user.nick, user.name, message.send, message.delete,
        message.edit, xp

        Note:
        Pastikan kamu sudah memilih channel, output log akan diterapkan pada channel yang kamu pilih.
        `acx!setlogchannel [channel]` """
        if not await Utils.is_bot_admin_reply(ctx): return
        
        if options == None:
            em = discord.Embed(color = 0XFF8C00, description = "> **Panduan Penggunaan**\n"
                                                               "> `{}logenable [option1, oprion2,...]`\n> \n"
                                                               "> **OPTIONS LIST**\n"
                                                               "> ```{}```\n"
                                                               "> Note:\n"
                                                               "> *Pastikan kamu sudah memilih channel, output log akan diterapkan pada channel yang kamu pilih.*\n"
                                                               ...
                                                               .format(ctx.prefix,
                                                                       ', '.join(self.logvars)))
            em.set_author(name = "Help command logenable", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        ...
        if not len(addedOptions):
            em = discord.Embed(color = 0XFF8C00, description = "> Kamu memasukan option yang salah.\n"
                                                               "> Ketik `{}help logenable` untuk melihat option.\n> \n"
                                                               "> Atau option yang kamu pilih sudah aktif.\n"
                                                               "> Ketik `{}logging` untuk melihat option yang sudah aktif"
                                                               .format(ctx.prefix,
                                                                       ctx.prefix))
            em.set_author(name = "logenable gagal", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
            em.set_footer(text = "Request by : {}".format(ctx.author.name), icon_url = "{}".format(ctx.author.avatar_url))
            return await ctx.send(embed = em)
        ...
        await ctx.send("*{}* logging option{} dinyalakan.".format(len(addedOptions),"" if len(addedOptions) == 1 else ""))
        
                
    @commands.command(pass_context=True)
    async def logdisable(self, ctx, *, options = None):
        """Menonaktifkan log, dengan pilihan option yang dipisahkan oleh koma(bot-admin only).
        jika tidak memasukan option, bot akan mematikan semua log yang sedang aktif.
        ...
        if options == None:
            msg = 'Menonaktifkan semua option logging yang sedang aktif.'
        ...
        if not len(addedOptions):
            return await ctx.send('┐(￣ヘ￣;)┌\nTidak ada log yang harus dimatikan.')

        # Save the updated options
        self.settings.setServerStat(ctx.guild, "LogVars", serverOptions)

        await ctx.send("*{}* logging option{} dimatikan.".format(len(addedOptions),"" if len(addedOptions) == 1 else ""))       
            
            
    @commands.command(pass_context=True)
    async def setlogchannel(self, ctx, *, channel : discord.TextChannel = None):
        """Mengatur channel untuk output log (bot-admin only)."""
        if not await Utils.is_bot_admin_reply(ctx): return

        if channel == None:
            self.settings.setServerStat(ctx.guild, "LogChannel", "")

            return await ctx.send('status logging saat ini *dimatikan*.')

        # If we made it this far - then we can add it
        self.settings.setServerStat(ctx.guild, "LogChannel", channel.id)
        em = discord.Embed(color = 0XFF8C00, description = "> Status logging saat ini *dinyalakan*\n"
                                                           "> channel output: **{}**"
                                                           .format(channel.mention))
        em.set_author(name = "LogChannel", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        em.set_footer(text = "Request by : {}".format(ctx.author.name), icon_url = "{}".format(ctx.author.avatar_url))
    ...
    async def setdebug(self, ctx, *, debug = None):
        """Set on/off debugging (owner only)"""
        # Only allow owner
        ...
        if debug == None:
            # Output debug status
            return await ctx.send("Debugging telah {}.".format("dinyalakan" if self.debug else "dimatikan"))
        elif debug.lower() in [ "yes", "on", "true", "ya"]:
            debug = True
        elif debug.lower() in [ "no", "off", "false", "tidak"]:
            debug = False
        else:
            debug = None

        if debug == True:
            msg = 'status Debugging masih menyala.' if self.debug == True else 'status Debugging dinyalakan.'
        else:
            msg = 'status Debugging sudah dimatikan.' if self.debug == False else 'status Debugging dimatikan.'
        self.debug = debug
    ...
    async def cleardebug(self, ctx):
        """Menghapus file debug.txt (owner only)."""
        # Only allow owner
        isOwner = self.settings.isOwner(ctx.author)
        if isOwner == None:
            return
        ...
        if not os.path.exists('debug.txt'):
            return await ctx.send("tidak ada *debug.txt* yang ditemukan.")
        # Exists - remove it
        os.remove('debug.txt')
        await ctx.send('*debug.txt* telah dihapus!')


    @commands.command(pass_context=True)
    async def heartbeat(self, ctx):
        """Menulis didalam console dan mengirim pesan (owner only)."""
        # Only allow owner
        isOwner = self.settings.isOwner(ctx.author)
        if isOwner == None:
            return
        ...
        # Message send
        message = await ctx.send('Heartbeat telah ditest {}.'.format(timeStamp))
