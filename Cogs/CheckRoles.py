        ...
        if current_role == target_role:
            ...
            if is_higher:
                if xpDemote:
                    ...
                    msg = '*{}* nRole kamu telah dihapus **{}**!'.format(DisplayName.name(user), current_role.name)
                    for role in promoArray:
                        test_role = DisplayName.roleForID(role['ID'], server)
                        if test_role != target_role and test_role in user.roles:
                            remRoles.append(test_role)
            ...
            # Promote if needed
                ...
                if xpPromote:
                    ...
                    msg = '*{}* kamu telah mendapatkan role **{}**!'.format(DisplayName.name(user), target_role.name)
                    for role in promoArray:
                        test_role = DisplayName.roleForID(role['ID'], server)
                        if test_role != target_role and test_role in user.roles:
                            remRoles.append(test_role)
                ...
                if int(role['XP']) <= userXP:
                    ...
                    if addRole:
                        if not addRole in user.roles:
                            ...
                            if not suppProm:
                                msg = '*{}* kamu telah mendapatkan role **{}**!'.format(DisplayName.name(user), addRole.name)
                            changed = True
                ...
                if int(role['XP']) > userXP:
                    ...
                    if remRole:
                        if remRole in user.roles:
                            ...
                            if not suppDem:
                                msg = '*{}*.\nRole kamu **{}** telah dihapus!'.format(DisplayName.name(user), remRole.name)
                            changed = True
