...
class JazUtils(commands.Cog):
    ...
    async def rolelist_paginate(self, ctx, rlist, title='Role List'):
        ...
        for index,page in enumerate(pages): # enumerate so we can add a page number
            embed = discord.Embed(title='{} halaman {:,}/{:,}'.format(title, index+1, len(pages)), description=page)
            await ctx.send(embed=embed)

    @commands.command(pass_context=True)
    async def roles(self, ctx, sort_order:str='name'):
        """Mnampilkan role and jumlah member yang mempunyai role tersebut.
        ...
        confirm = ['ya']
        Msg = await ctx.send("Command ini mungkin akan membuat list embed yang panjang.\nKetik `ya` untuk melanjutkan")
        try:
        ...
        if message not in confirm and message not in ['ya']:
            await Msg.delete()
            return await ctx.send(f'{ctx.author.name}\nCommand dibatalkan.\nKamu memasukan konfirmasi yang salah', delete_after=10)

        if not sort_order in ['default', 'name', 'count', 'rainbow']: # make sure it has valid args
            return await ctx.send("Argument yang kamu masukan salah.\nKetik `{}help roles` untuk bantuan.")
    ...
    async def whohas(self, ctx, *, rolename):
        ...
        embed.set_footer(text='ROLE ID: {}\nRequest By : {}'.format(check_role.id, ctx.author.name))
        await ctx.send(embed=embed)
    ...
    async def rolecount(self, ctx):
        ...
        em.set_footer(text = "SERVER ID : {}\nRequest By : {}".format(ctx.guild.id, ctx.author.name))
    ...
    async def emptyroles(self, ctx):
        ...
        if not sorted_list: # another failsafe
            return await ctx.send('Sepertinya tidak ada role yang kosong...')
