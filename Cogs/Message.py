    ...
    async def _send_message(self, ctx, message, pm = False, file_path = None):
        ...
        if not file_path == None:
            ...
            if not dfile:
                # File doesn't exist...
                try:
                    await ctx.send("┐(￣ヘ￣;)┌ Terjadi kesalahan!\nFile tidak dapat dikirim")
                except:
                    # We tried...
                    pass
                return None
        ...
        if pm == True and type(ctx) is discord.ext.commands.Context and not ctx.channel == ctx.author.dm_channel:
            ...
            try:
                ...
                if self.force_pm:
                    ...
                    try:
                        await ctx.send("┐(￣ヘ￣;)┌ Terjadi kesalahan!\nTidak dapat mengirim pesan ini ke private message mu")
                    except:
                        # We tried...
                        pass
                    return None
...
class Embed:
    ...
    async def _send_embed(self, ctx, embed, pm = False, file_path = None):
        ...
        if not file_path == None:
            ...
            if not dfile:
                # File doesn't exist...
                try:
                    await Embed(title="An error occurred!", description="┐(￣ヘ￣;)┌\nFile tidak dapat dikirim...", color=self.color).send(ctx)
                except:
                    # We tried...
                    pass
                return None
        ...
        if pm == True and type(ctx) is discord.ext.commands.Context and not ctx.channel == ctx.author.dm_channel:
            ...
            try:
                ...
                if self.force_pm:
                    # send an error embed
                    try:
                        await Embed(title="An error occurred!", description="┐(￣ヘ￣;)┌\nAku tidak dapat pengirim pesan DM pada mu", color=self.color).send(ctx)
                    except:
                        # We tried...
                        pass
                    return None
    ...
    async def edit(self, ctx, message):
        ...
        if new_message.channel == ctx.author.dm_channel and not ctx.channel == ctx.author.dm_channel:
            em = Embed(title=self.title, description="📬 Cek pesan DM kamu", color=self.color)._embed_with_self()
            await message.edit(content=None, embed=em, delete_after=self.delete_after)
        else:
            await message.delete()
            # await message.edit(content=" ", embed=None, delete_after=self.delete_after)
    ...
    async def send(self, ctx):
        ...
        if page_total > 1 and self.page_count and self.title:
            add_title = " (Halaman {:,} dari {:,})".format(page_count, page_total)
