...
class LangFilter(commands.Cog):
    ...
    async def addfilter(self, ctx, *, words = None):
        ...
        if not len(addedOptions):
            return await ctx.send('Tidak ada kata baru yang ditambahkan.')
        ...
        if len(addedOptions) == 1:
            await ctx.send('*1* kata telah ditambahkan kedalam chat filter list.')
        else:
            await ctx.send('*{}* kata telah ditambahkan kedalam chat filter list.'.format(len(addedOptions)))
    ...
    async def remfilter(self, ctx, *, words = None):
        ...
        for option in optionList:
            ...
            if option.lower() in serverOptions:
                ...
                addedOptions.append(option.lower())
        if not len(addedOptions):
            return await ctx.send('Aku tidak menemukan kata itu dalam list.')
        ...
        if len(addedOptions) == 1:
            await ctx.send('*1* kata telah di hapus dari filter chat.')
        else:
            await ctx.send('*{}* kata telah di hapus dari filter chat.'.format(len(addedOptions)))
    ...
    async def clearfilter(self, ctx):
        ...
        if len(serverOptions) == 1:
            ...
            await ctx.send('*{}* kata telah di hapus dari filter chat.'.format(len(serverOptions)))
