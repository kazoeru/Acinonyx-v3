...
class Strike(commands.Cog):
    ...
    async def strike(self, ctx, member : discord.Member = None, days = None, *, message : str = None):
        """Memberikan strike kepada user dalam server (admin only)."""
        ...
        if member.id == ctx.message.author.id:
            ...
            msg = '┐(￣ヘ￣;)┌\nBodoh..! Kamu tidak dapat memberikan strike pada dirimu sendiri.'
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
            await ctx.channel.send(embed = em)
            return
            ...
            for role in member.roles:
                for aRole in checkAdmin:
                    ...
                    if str(aRole['ID']) == str(role.id):
                        isAdmin = True
        if isAdmin:
            msg = '┐(￣ヘ￣;)┌\nKamu tidak dapat memberikan strike kepada admin yang lainnya.'
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
            await ctx.channel.send(embed = em)
            return
        ...
        if strikeNum < strikeout:
            ...
            if strikeLevel == 0:
                ...
                em = discord.Embed(color = 0XFF8C00, description = mutemessage)
                em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
                ...
                em = discord.Embed(color = 0XFF8C00, description = kickmessage)
                em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
                await member.send(embed = em)
                ...
                em = discord.Embed(color = 0XFF8C00, description = banmessage)
                em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
                await member.send(embed = em)
    ...
    async def strikeinfo(self, ctx, *, member = None):
        """Melihat info strike diri sendiri/user lain (hanya user dengan permission administrator dapat melihat strike user lain)."""
        isAdmin = ctx.message.author.permissions_in(ctx.message.channel).administrator
        if not isAdmin:
            checkAdmin = self.settings.getServerStat(ctx.message.guild, "AdminArray")
            for role in ctx.message.author.roles:
        ...
        stat_embed.add_field(name="Strike", value=len(strikes), inline=True)
    ...
    async def removestrike(self, ctx, *, member = None):
        """Menghapus strike yang telah diberikan kepada member (admin only)."""
        isAdmin = ctx.message.author.permissions_in(ctx.message.channel).administrator
        if not isAdmin:
            checkAdmin = self.settings.getServerStat(ctx.message.guild, "AdminArray")
            for role in ctx.message.author.roles:
        ...
        if member == None:
            em = discord.Embed(color = 0XFF8C00,
                               description = "> Menghapus strike yang telah diberikan kepada member (admin only)\n"
                                             "> **Panduan**\n"
                                             "> `{}removestrike [member]`"
                                             .format(ctx.prefix))
        ...
        if not len(strikes):
            msg = '*{}* tidak memiliki strike yang telah di terima.'.format(DisplayName.name(member))
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
            await ctx.channel.send(embed = em)
            return
        ...
        for strike in strikes:
            ...
            if not strike['Time'] == -1:
                ...
                strikes.remove(strike)
                self.settings.setUserStat(member, ctx.message.guild, "Strikes", strikes)
                msg = 'Strike untuk *{}* telah dihapus. Tersisa *{}* strike.'.format(DisplayName.name(member), len(strikes))
                em = discord.Embed(color = 0XFF8C00, description = msg)
                em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
                await ctx.channel.send(embed = em)
                return
    ...
    async def setstrikelevel(self, ctx, *, member = None, strikelevel : int = None):
        """Mengatur level strike untuk user (admin only)."""
        ...
        if self.settings.getServerStat(server, "SuppressMentions"):
            ...
            suppress = False
        em = discord.Embed(color = 0XFF8C00,
                           description = "> Mengatur level strike untuk user (admin only)\n"
                                         "> **Panduan**\n"
    ...
    async def setstrikelimit(self, ctx, limit = None):
        """Mengatur batas strike limit untuk ditindaklanjuti (admin only)."""
        isAdmin = ctx.message.author.permissions_in(ctx.message.channel).administrator
        if not isAdmin:
            checkAdmin = self.settings.getServerStat(ctx.message.guild, "AdminArray")
            for role in ctx.message.author.roles:
        ...
        if not limit:
            msg = 'Strike limit minimum adalah 1.'
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
            await ctx.channel.send(embed = em)
            return
        ...
        try:
            ...
            msg = 'Strike limit haru berupa angka.'
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
            await ctx.channel.send(embed = em)
            return
    ...
    async def setstrikelimit_error(self, ctx, error):
        ...
        em = discord.Embed(color = 0XFF8C00, description = msg)
        em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
        await error.channel.send(embed = em)
