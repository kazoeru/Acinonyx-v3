...
class Mute(commands.Cog):
    ...
    async def _ask_perms(self, ctx, mute_role, desync=False, show_count=True):
        ...
        try: sync_response = await self.bot.wait_for('message', timeout=60, check=check_answer)
            ...
            await ctx.send("Waktu kamu habis.")
            return None
        if sync_response.content.lower() in ["n","no","false"]:
            await ctx.send("Permissions untuk **{}** dibatalkan.".format(Utils.suppressed(ctx,mute_role.name)))
            return False
        # We want to adjust perms
        syncing_message = await ctx.send("{}yncing permissions untuk **{}**...".format("Des" if desync else "S", Utils.suppressed(ctx,mute_role.name)))
        await self._sync_perms(ctx,mute_role,desync)
        await syncing_message.edit(content="**{}** telah **{}ynced**.".format(Utils.suppressed(ctx,mute_role.name),"de" if desync else "s"))
        return True
    ...
    async def setmuterole(self, ctx, *, role = None):
        """Mengatur target role untuk mute(bot-admin/server-owner only).
        ...
        if role:
            ...
            em = discord.Embed(color = 0XFF8C00, description =  "> Role *{}* tidak ditemukan.\n"
                                                                ...
                                                                .format(role,
                                                                        ctx.prefix))
            em.set_author(name = "Oops ada yang salah!", url = "https://acinonyxesports.com/", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        ...
        await ctx.send("Mute role saat ini: **{}**".format(Utils.suppressed(ctx,mute_role.name)) if mute_role else "Tidak ada **mute role** yang telah disetup.")
        if role == None:
            if mute_role:
                await self._ask_perms(ctx,mute_role,desync=True,show_count=True)
            self.settings.setServerStat(ctx.guild,"MuteRole",None)
            return await ctx.send("Mute role **dihapus**\nSystem mute sekarang akan ditulis per channel!") if mute_role else None
        if mute_role:
            if mute_role == target_role:
                await ctx.send("Mute role **masih sama** dengan sebelumnya!")
        ...
        await ctx.send("Mute role telah di set ke **{}**!".format(Utils.suppressed(ctx,target_role.name)))

    @commands.command(pass_context=True)
    async def muterole(self, ctx):
        """Melihat role mute yang telah di setting dalam server (bot-admin/server-owner only)."""
        if not await Utils.is_bot_admin_reply(ctx): return
        ...
        await ctx.send("User yang di mute akan diberi role **{}**.".format(Utils.suppressed(ctx,mute_role.name)))

    @commands.command(pass_context=True)
    async def createmuterole(self, ctx, *, role_name = None):
        """Mengatur mute role saat melakukan mute untuk member (bot-admin/server-owner only)."""
        if not await Utils.is_bot_admin_reply(ctx): return
        ...
        if mute_role: # Already exists - let's update the settings
            ...
            return await ctx.send("Mute role telah di set ke **{}**!".format(Utils.suppressed(ctx,mute_role.name)))
        # Create a role with the proper overrides
        message = await ctx.send("Membuat role **{}**...".format(Utils.suppressed(ctx,role_name)))
        ...
        except Exception as e: return await message.edit(content="Pembuatan Role gagal dengan error:\n```\n{}\n```".format(e))
        # Walk the channels and set the perms for the role
        await message.edit(content="Updating permissions **{}**...".format(Utils.suppressed(ctx,role_name)))
        ...
        self.settings.setServerStat(ctx.guild,"MuteRole",mute_role.id)
        await message.edit(content="User yang di mute akan diberikan role **{}**.".format(Utils.suppressed(ctx,mute_role.name)))

    @commands.command(pass_context=True)
    async def syncmuterole(self, ctx):
        """Memastikan mute role memiliki pengaturan send_messages, add_reactions, dan speak_overrides yang di nonaktifkan dalam semua channel (bot-admin only)."""
        if not await Utils.is_bot_admin_reply(ctx): return
        role = self.settings.getServerStat(ctx.guild,"MuteRole")
        if not role:
            em = discord.Embed(color = 0XFF8C00, description =  "> Tidak ada mute role yang telah di setup.\n"
        ...
        message = await ctx.send("Syncing permissions untuk **{}**...".format(Utils.suppressed(ctx,mute_role.name)))
        await self._sync_perms(ctx,mute_role)
        await message.edit(content="mute role **{}** telah berhasil di sinkronisasi.".format(Utils.suppressed(ctx,mute_role.name)))

    @commands.command(pass_context=True)
    async def desyncmuterole(self, ctx):
        """Menghapus send_messages, add_reactions, dan speak_overrides dari mute role(Bot-admin/owner-server only).
        ...
        await message.edit(content="Role mute **{}** telah **matikan**.\nTidak akan mute member!".format(Utils.suppressed(ctx,mute_role.name)))

    @commands.command(pass_context=True)
    async def mute(self, ctx, *, member = None, cooldown = None):
        """Mencegah member untuk mengirim pesan dalam chat atau berbicara dalam voice channel(bot-admin/server-owner only)."""
        if not await Utils.is_bot_admin_reply(ctx): return
        if member == None:
        ...
        if member is ctx.author:
            msg = 'Akan lebih mudah untuk ku kalo kamu yang diam sendiri!'
            return await ctx.send(msg)
        # Check if we're muting the bot
        if member.id == self.bot.user.id:
            msg = 'Mute aku sendiri?\nOgah banget...'
            return await ctx.send(msg)
        ...
        if type(cooldown) is int or type(cooldown) is float:
            if cooldown < 0:
                msg = 'Cooldown tidak dapat dilakukan dengan angka negatif!'
                return await ctx.send(msg)
            currentTime = int(time.time())
    ...
    async def unmute(self, ctx, *, member = None):
        """Membuka blokir mute member (bot-admin/server-owner only)."""
        if not await Utils.is_bot_admin_reply(ctx): return
        if member == None:
        ...
        mess = await ctx.send("Membuka mute...")
    ...
    async def ismuted(self, ctx, *, member = None):
        ...
        if mute_role and mute_role in member.roles:
            ...
            if not item or item["Cooldown"] == None:
                ...
                return await ctx.send("*{}* telah di **mute**.".format(DisplayName.name(member)))
            # We're still muted - but have a cooldown
            return await ctx.send("*{}* telah di mute **muted**.\n*{}* waktu tersisa.".format(
        ...
        if muted_channels:
            if not item or item["Cooldown"] == None:
                # No cooldown - let's just give the bad news...
                return await ctx.send("*{}* telah di **mute** dalam channel {}{}.".format(
                    DisplayName.name(member),
                    muted_channels,
                    "" if muted_channels == 1 else ""
                ))
            # We're still muted - but have a cooldown
            return await ctx.send("*{}* telah di **mute** dalam channel {}{},\n*{}* waktu tersisa.".format(
                DisplayName.name(member),
                muted_channels,    
        ...
        await ctx.send("*{}* telah di **unmute**.".format(DisplayName.name(member)))
