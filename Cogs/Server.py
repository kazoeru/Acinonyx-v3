...
class Server(commands.Cog):
    ...
    async def getprefix(self, ctx):
        ...
        Mention bot ini jika kalian lupa prefix dalam server mu"""
    ...
    async def leaveserver(self, ctx, *, targetServer = None):
        """Pergi dari server, membutuhkan nama/id server (owner only)."""
        if not await Utils.is_owner_reply(ctx): return
        ...
        if guild:
            await guild.leave()
            try:
                await ctx.send("Aku sudah pergi dari server itu.")
            except:
                pass
            return
