...
class RateLimit(commands.Cog):
    ...
    async def ccooldown(self, ctx, delay : int = None):
        ...
        if delay == None:
            if currDelay == 1:
                await ctx.channel.send('Cooldown untuk semua command telah dirubah menjadi *1 Detik*')
            else:
                await ctx.channel.send('Cooldown untuk semua command telah dirubah menjadi *{} Detik.*'.format(currDelay))
            return
        
        try:
            delay = int(delay)
        except Exception:
            await ctx.channel.send('Cooldown harus berupa angka.')
            return
        
        if delay < 0:
            await ctx.channel.send('Cooldown harus lebih dari *0 detik*.')
            return

        if delay > self.maxCooldown:
            if self.maxCooldown == 1:
                await ctx.channel.send('Cooldown tidak dapat lebih dari *1 detik*.')
            else:
                await ctx.channel.send('Cooldown tidak dapat lebih dari *{} detik*.'.format(self.maxCooldown))
            return
        ...
        if delay == 1:
            ...
            await ctx.channel.send('Cooldown untuk semua command telah dirubah menjadi *{} Detik.*'.format(delay))
