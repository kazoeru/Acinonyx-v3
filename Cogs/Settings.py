...
class Settings(commands.Cog):
    ...
    async def dumpsettings(self, ctx):
        """Mengirim setting.json (owner-only)."""
        ...
        message = await ctx.send('Mengunggah *Settings.json*...')
    ...
    async def addowner(self, ctx, *, member : str = None):
        """Menambahkan owner baru untuk bot ini (MAIN owner-only)."""
        ...
        if type(member) is str:
            ...
            if memberCheck:
                ...
                msg = '┐(￣ヘ￣;)┌\nAku tidak dapat menemukan user yang kamu maksud...'
                await ctx.channel.send(msg)
                return
        
        if member.bot:
            msg = "┐(￣ヘ￣;)┌\nAku tidak mau bot lain menjadi owner ku."
            await ctx.channel.send(msg)
            return
        owners = self.getOwners()
        if member.id in owners:
            # Already an owner
            msg = "┐(￣ヘ￣;)┌\nBodoh... *{}* sudah menjadi owner ku.".format(DisplayName.name(member))
        else:
            owners.append(member.id)
            self.serverDict['Owner'] = owners
            msg = '*{}* telah ditambahkan kedalam list owner ku!'.format(DisplayName.name(member))
        await ctx.channel.send(msg)


    @commands.command(pass_context=True)
    async def remowner(self, ctx, *, member : str = None):
        """Menghapus owner tambahan dari list (MAIN owner-only)."""
        ...
        if member.id in owners:
            # Already an owner
            msg = "*{}* telah dihapus dari list owner ku.".format(DisplayName.name(member))
            owners.remove(member.id)
            self.serverDict['Owner'] = owners
        else:
            msg = "┐(￣ヘ￣;)┌\n*{}* tidak terdaftar di list owner ku".format(DisplayName.name(member))
        if not len(self.serverDict['Owner']):
            # No more owners
            msg += " Aku tidak memiliki owner!"
    ...
    async def flush(self, ctx):
        """Flush setting bot ke disk directory local (admin only)."""
        # Only allow owner
        isOwner = self.isOwner(ctx.author)
        if isOwner == None:
            return
            ...
            return
        # Flush settings
        message = await ctx.send("Flush setting ke disk local...")
        ...
        msg = 'Setting telah di simpan dalam disk local.'
    ...
    async def prunelocalsettings(self, ctx):
        ...
        message = await ctx.send("Menghapus semua setting dalam server saat ini...")
        ...
        await message.edit(content="Mengubah dan menyimpan standar setting ke disk local...", embed=None)
    ...
    async def prunesettings(self, ctx):
        ...
        message = await ctx.send("Mencoba reset semua setting di semua server...")
        ...
        await message.edit(content="Mengubah dan menyimpan standar setting ke disk local...")
        # Actually flush settings asynchronously here
        l = asyncio.get_event_loop()
        await self.bot.loop.run_in_executor(None, self.flushSettings, self.file, True)
    ...
    async def prune(self, ctx):
        ...
        message = await ctx.send("Mengecek member dalam list dan mencoba untuk menghapusnya...")
        ...
        await message.edit(content="Menyimpan pengaturan baru ke disk local...")
        # Actually flush settings asynchronously here
        l = asyncio.get_event_loop()
        await self.bot.loop.run_in_executor(None, self.flushSettings, self.file, True)
