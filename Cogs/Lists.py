...
class Lists(commands.Cog):
    ...
    async def online(self, ctx):
        ...
        await Message.Embed(
            title="Member Stats",
            description="> Informasi online member untuk server **{}**".format(ctx.guild.name),
            fields=[
            ...
            color=0XFF8C00,
            footer="Request by: {}".format(ctx.author.name)).send(ctx)
    ...
    async def lastonline(self, ctx, *, member = None):
        ...
        if not member:
            em = discord.Embed(color = 0XFF8C00, description = "> Melihat informasi member terakhir online\n> \n"
                                                                   ...
                                                                   .format(ctx.prefix,
                                                                           ctx.prefix))
            em.set_author(name = "Last Online", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        ...
        if not member.status == discord.Status.offline:
            ...
            if lastOnline:
                ...
                msg = 'Terakhir aku lihat *{}* online *{} yang lalu*.'.format(name, timeString )
            else:
                msg = '┐(￣ヘ￣;)┌\nAku tidak tau kapan *{}* terakhir online.'.format(name)
        await ctx.send(msg)
