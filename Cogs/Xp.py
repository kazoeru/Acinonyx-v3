...
class Xp(commands.Cog):
        ...
        if approve:
            ...
            if isRole:
                ...
                if len(memSorted):
                    ...
                    msg = '┐(￣ヘ￣;)┌\nSepertinya member *{}* tidak memenuhi syarat!*'.format(member.name)
                    if suppress:
                        msg = Nullify.clean(msg)
                    em = discord.Embed(color = 0XFF8C00, description = msg)
                    em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
                ...
                msg = '*{}* telah memberikan *{:,} xp!*'.format(DisplayName.name(member), xpAmount)
                # Check for suppress
                if suppress:
                    msg = Nullify.clean(msg)
                em = discord.Embed(color = 0XFF8C00, description = msg)
    ...
    async def defaultrole(self, ctx):
        """Melihat default role yang akan diberikan kepada user baru."""
        ...
        if role == None or role == "":
            msg = 'Tidak ada default role yang telah kamu setting.\n'
            msg += 'User yang baru bergabung tidak akan diberikan role pada server ini.\n'
            msg += 'Kamu dapat menambahkan defult role dengan mengetik\n'
            msg += '*`{}setdefaultrole [nama_role]`*'.format(ctx.prefix)
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
            ...
            for arole in ctx.message.guild.roles:
                if str(arole.id) == str(role):
                    ...
                    if suppress:
                        msg = Nullify.clean(msg)
            if not found:
                msg = '┐(￣ヘ￣;)┌ Tidak ada role yang cocok dengan ID: `{}`\nCobalah untuk mengatur ulang settingan mu.\n*`{}setdefaultrole [nama_role]`*'.format(role, ctx.prefix)
            em = discord.Embed(color = 0XFF8C00, description = msg)
            em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
            await ctx.message.channel.send(embed = em)
    ...
    async def gamble(self, ctx, bet : int = None):
        ...
        if isAdmin:
            ...
            if adminUnlim:
                ...
                if bet > int(reserveXP):
                    # Don't approve if we don't have enough
                    msg = '┐(￣ヘ￣;)┌\nKamu tidak dapat bertaruh dengan jumlah *{:,} Xp*, kamu hanya memiliki *{:,} Xp* yang tersimpan!'.format(bet, reserveXP)
                    approve = False
        ...
        if approve:
            ...
            if randnum == 1:
                ...
                msg = '*{}* bertaruh dengan *{:,} Xp* dan ***MEMENANGKAN*** *{:,} Xp!*\nTotal : *{} Xp!*'.format(DisplayName.name(author), bet, int(payout), bet*2)
                # Now we check for promotions
                await CheckRoles.checkroles(author, channel, self.settings, self.bot)
            else:
                msg = '*{}* bertaruh dengan *{:,} Xp* dan.... *tidak* memenangkan apapun.\nSemoga beruntung lain waktu!'.format(DisplayName.name(author), bet)
        em = discord.Embed(color = 0XFF8C00, description = msg)
        em.set_footer(text = "{}".format(ctx.author), icon_url = "{}".format(ctx.author.avatar_url))
        await ctx.message.channel.send(embed = em)
