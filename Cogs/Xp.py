...
class Xp(commands.Cog):
    ...
    async def xp(self, ctx, *, member = None, xpAmount : int = None):
        """Memberikan XP kepada member atau role lain dalam server mu."""
        ...
        if self.settings.getServerStat(server, "SuppressMentions"):
            ...
            suppress = False
        em = discord.Embed(color = 0XFF8C00, description = "> Memberikan XP ke user lain.\n> \n"
                                                           "> **Panduan**\n"
                                                           "> `{}xp [role/member] [jumlah]`"
                                                           .format(ctx.prefix))
        em.set_author(name = "xp Command", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        ...
        if not self._can_xp(author, server):
            approve = False
            msg = '┐(￣ヘ￣;)┌\nKamu tidak memiliki izin untuk memberikan xp.'

        if xpAmount > int(reserveXP):
            approve = False
            msg = '┐(￣ヘ￣;)┌\nKamu tidak memiliki *{:,} xp*, xp mu hanya tersisa *{:,}!*'.format(xpAmount, reserveXP)

        if author == member:
            approve = False
            msg = '┐(￣ヘ￣;)┌\nPercobaan yang bagus, tapi aku memiliki system kamu tidak akan dapat memberikan xp kepada dirimu sendiri...*'

        if xpAmount < 0:
            msg = '┐(￣ヘ￣;)┌\nHanya admin yang dapat mengurangi xp!'
            approve = False
            # Avoid admins gaining xp
            decrement = False

        if xpAmount == 0:
            msg = 'Wow, baik sekali yaa...'
            approve = False
        ...
        if type(member) is discord.Role:
            if member.id in xpblock:
                msg = "┐(￣ヘ￣;)┌\nRole tersebut tidak dapat menerima xp!"
                approve = False
        else:
            # User
            if member.id in xpblock:
                msg = "┐(￣ヘ￣;)┌\nMember itu tidak dapat menerima xp!"
                approve = False
            else:
                for role in member.roles:
                    if role.id in xpblock:
                        msg = "┐(￣ヘ￣;)┌\nMember role tidak dapat menerima xp!"
                        approve = False
        
        if ctx.author.id in xpblock:
            msg = "┐(￣ヘ￣;)┌\nKamu tidak dapat memberikan xp!"
            approve = False
        else:
            for role in ctx.author.roles:
                if role.id in xpblock:
                    msg = "┐(￣ヘ￣;)┌\nRole kamu tidak dapat memberikan xp!"
                    approve = False
        ...
        if approve:
            ...
            if isRole:
                ...
                if len(memSorted):
                    ...
                    msg = '*{:,} xp* telah diberikan kepada *{}!*'.format(totalXP, member.name)
                    # Check for suppress
                    if suppress:
                        msg = Nullify.clean(msg)
                    await channel.send(msg)
                else:
                    msg = '┐(￣ヘ￣;)┌\nTidak ada member yang memenuhi syarat di *{}!*'.format(member.name)
                    if suppress:
                        msg = Nullify.clean(msg)
                    await channel.send(msg)
    ...
    async def defaultrole(self, ctx):
        """Melihat default role yang ditetapkan untuk user baru."""
        ...
        if role == None or role == "":
            msg = 'User yang baru bergabung tidak akan diberikan role pada server ini.\n'
            await ctx.channel.send(msg)
        else:
            ...
            for arole in ctx.message.guild.roles:
                if str(arole.id) == str(role):
                    found = True
                    msg = 'User baru akan diberikan role **{}**.'.format(arole.name)
                    # Check for suppress
                    if suppress:
                        msg = Nullify.clean(msg)
            if not found:
                msg = '┐(￣ヘ￣;)┌ Tidak ada role yang cocok dengan ID: `{}`\nCobalah untuk mengatur ulang settingan mu.'.format(role)
            await ctx.message.channel.send(msg)
        
    @commands.command(pass_context=True)
    async def gamble(self, ctx, bet : int = None):
        """Bertaruh dengan xp yang kamu miliki dan menangkan xp lebih banyak!"""
        ...
        em = discord.Embed(color = 0XFF8C00, description = "> Bertaruh dengan xp yang kamu miliki dan menangkan xp lebih banyak!\n> \n"
                                                           "> **Panduan**\n"
                                                           "> `{}gamble [jumlah xp] (harus kelipatan 10)`"
                                                           .format(ctx.prefix))
        em.set_author(name = "┐(￣ヘ￣;)┌", url = "https://acinonyxesports.com", icon_url = "https://cdn.discordapp.com/attachments/518118753226063887/725569194304733435/photo.jpg")
        em.set_thumbnail(url = "{}".format(ctx.message.guild.icon_url))
        em.set_footer(text = "Saat mengetik command, tanda [] tidak usah digunakan\nRequest by : {}".format(ctx.author.name), icon_url = "{}".format(ctx.author.avatar_url))
        msg = em
        ...
        if not bet % 10 == 0:
            approve = False
            msg = '┐(￣ヘ￣;)┌\nTaruhan gamble harus dalam kelipatan *10!*'
            
        if bet > int(reserveXP):
            approve = False
            msg = '┐(￣ヘ￣;)┌\nKamu tidak dapat bertaruh dengan jumlah *{:,}*, kamu hanya memiliki *{:,}* xp yang tersimpan!'.format(bet, reserveXP)
            
        if bet < 0:
            msg = '┐(￣ヘ￣;)┌\nKamu tidak dapat memasukan dengan nominal negatif!'
            approve = False
            
        if bet == 0:
            msg = '┐(￣ヘ￣;)┌\nKamu **cerdas sekali** sekali!'
            approve = False

        # RequiredXPRole
        if not self._can_xp(author, server):
            approve = False
            msg = 'Kamu tidak memiliki izin untuk menggunakan gamble.'
        ...
        # Check if we're blocked
        if ctx.author.id in xpblock:
            msg = "┐(￣ヘ￣;)┌\nKamu tidak dapat melakukan gamble xp!"
            approve = False
        else:
            for role in ctx.author.roles:
                if role.id in xpblock:
                    msg = "┐(￣ヘ￣;)┌\nRole kamu tidak dapat melakukan gamble xp!"
                    approve = False
        ...
        if approve:
            ...
            if randnum == 1:
                ...
                msg = '*{}* bertaruh dengan *{:,} xp* dan ***MEMENANGKAN*** *{:,} xp!*'.format(DisplayName.name(author), bet, int(payout))
                # Now we check for promotions
                await CheckRoles.checkroles(author, channel, self.settings, self.bot)
            else:
                msg = '*{}* bertaruh dengan *{:,}* dan.... *tidak* memenangkan apapun.\nSemoga beruntung lain waktu!'.format(DisplayName.name(author), bet)
            
        await ctx.message.channel.send(msg)
            
    @commands.command(pass_context=True)
    async def recheckroles(self, ctx):
        """Melakukan cek xp untuk semua role dan akan memberikan role kepada member yang memiliki xp yang sesuai yang mereka punya masing masing (admin only)."""
        ...
        xp_sd = self.settings.getServerStat(server,"SuppressDemotions")
        xp_oo = self.settings.getServerStat(server,"OnlyOneRole")
        message = await ctx.channel.send('Memeriksa role...')
        ...
        if changeCount == 1:
            await message.edit(content='Pengecekan role selesai.\n\n*1 user* telah diupdate.')
            #await channel.send('Done checking roles.\n\n*1 user* updated.')
        else:
            await message.edit(content='Pengecekan role selesai.\n\n*{:,} user* telah diupdate.'.format(changeCount))
            #await channel.send('Done checking roles.\n\n*{} users* updated.'.format(changeCount))

    @commands.command(pass_context=True)
    async def recheckrole(self, ctx, *, user : discord.Member = None):
        """Mengecek xp semua member dan memberikan role yang sesuai berdasarkan dengan xp yang mereka miliki (admin only)."""
        ...
        if await CheckRoles.checkroles(user, channel, self.settings, self.bot):
            await channel.send('Pengecekan role selesai.\n\n*{}* telah diupdate.'.format(DisplayName.name(user)))
        else:
            await channel.send('Pengecekan role selesai.\n\n*{}* tidak diupdate.'.format(DisplayName.name(user)))



    @commands.command(pass_context=True)
    async def listxproles(self, ctx):
        """Melihat semua list role, id, dan xp yang dibutuhkan untuk promotion/demotion xp system."""
        ...
        if not len(promoSorted):
            roleText = "Tidak ada role yang tersimpan dalam list role xp ku. Kamu dapat menambahkannya dengan command `{}addxprole [role] [jumlah_xp]`\n".format(ctx.prefix)
        else:
            roleText = "**__Role saat ini:__**\n\n"
            ...
            for arole in ctx.message.guild.roles:
                if str(arole.id) == str(role):
                    ...
                    if arole.name[:1].lower() in vowels:
                        roleText = '{}\nKamu membutuhkan role **{}** untuk *transfer xp*, *gamble*, atau *memberi* makan bot.'.format(roleText, arole.name)
                    else:
                        roleText = '{}\nKamu membutuhkan role **{}** untuk *transfer xp*, *gamble*, atau *memberi* makan bot.'.format(roleText, arole.name)
                    # roleText = '{}\nYou need to be a/an **{}** to give xp, gamble, or feed the bot.'.format(roleText, arole.name)
            if not found:
                roleText = '{}\nTidak ada role yang cocok dengan ID: `{}` untuk menggunakan XP system - Cobalah untuk update settingan mu.'.format(roleText, role)
    ...
    async def rank(self, ctx, *, member = None):
        """Cek ranking tertinggi dalam server mu."""
        ...
        if member.nick:
            # We have a nickname
            # Add to embed
            stat_embed.set_author(name='Siapa yang telah menyusul mu {}'.format(member.nick), icon_url=avURL)
        else:
            # Add to embed
        ...
        if highestRole == "":
            msg = '*{}* belum mendapatkan ranking.'.format(DisplayName.name(member))
            # Add Rank
            stat_embed.add_field(name="Rank saat ini", value='Belum diperoleh', inline=True)
        else:
            msg = '*{}* sekarang memiliki **{}**!'.format(DisplayName.name(member), highestRole)
            # Add Rank
            stat_embed.add_field(name="Rank saat ini", value=highestRole, inline=True)
    ...
    async def leaderboard(self, ctx, total : int = 10):
        """Melihat list top xp dalam server mu (max 50, default 10)."""
        message = await ctx.send("Menghitung xp...")
        promoArray = {}
        for x in self.bot.get_all_members():
    ...
    async def stats(self, ctx, *, member= None):
        """Melihat informasi member, xp dalam server ini, dll."""
        ...
        stat_embed.add_field(name="XP Tersimpan", value="{:,}".format(newState), inline=True)
        ...
        if member.nick:
            ...
            # Add to embed
            stat_embed.set_author(name='Informasi {}'.format(member.nick))
        else:
            msg = "__***{}:***__\n\n".format(member.name)
            # Add to embed
        ...
        if member.joined_at != None:
            ...
            # Add Joined
            stat_embed.add_field(name="Tanggal bergabung", value=j_time_str, inline=True)
        else:
            stat_embed.add_field(name="Tanggal bergabung", value="Unknown", inline=True)
        ...
        if highestRole:
            ...
            if len(promoSorted):
                ...
                msg = '{}Mereka belum mendapatkan ranking.\n'.format(msg)
                # Add Rank
                stat_embed.add_field(name="Rank saat ini", value='Belum mendapatkan ranking', inline=True)
        
        if nextRole and (newStat < int(nextRole['XP'])):
            # Get role
            next_role = DisplayName.roleForID(int(nextRole["ID"]), ctx.guild)
            if not next_role:
                next_role_text = "Role ID: {} (telah dihapus dari server)".format(nextRole["ID"])
            else:
                next_role_text = next_role.name
            msg = '{}\n*{:,}* *xp* lagi untuk mencapai role **{}**'.format(msg, int(nextRole['XP']) - newStat, next_role_text)
            # Add Next Rank
            stat_embed.add_field(name="Next Rank", value='{} (membutuhkan {:,} xp lagi)'.format(next_role_text, int(nextRole['XP'])-newStat), inline=True)
        ...
        if member.status == discord.Status.offline:
            ...
            status_text = "<:idle:768402412045729802> IDLE/AFK"
        ...
        if member.premium_since:
            ...
            stat_embed.add_field(name="Boost Nitro Sejak",value=c_time_str)
        ...
        if member.joined_at != None:
            ...
            stat_embed.add_field(name="Posisi Bergabung", value="{:,} dari {:,}".format(position, total), inline=True)
        else:
            stat_embed.add_field(name="Posisi Bergabung", value="Tidak diketahui", inline=True)
        
        # Get localized user time
        local_time = UserTime.getUserTime(ctx.author, self.settings, member.created_at, clock=False)
        c_time_str = "{} {}".format(local_time['time'], local_time['zone'])
        # add created_at footer
        created = "Tanggal Akun Dibuat " + c_time_str
