...
class XpStack(commands.Cog):
    ...
    async def clearallxp(self, ctx):
        """Menghapus semua xp dari seluruh member/role/server (owner-only)."""
        # Only allow owner
        ...
        await ctx.send("Semua xp user/role/server telah dihapus!")
    ...
    async def checkxp(self, ctx):
        """Menampilkan history transaksi xp dalam server (admin only)."""
        isAdmin = ctx.message.author.permissions_in(ctx.message.channel).administrator
        if not isAdmin:
            checkAdmin = self.settings.getServerStat(ctx.message.guild, "AdminArray")
            for role in ctx.message.author.roles:
        ...
        if not len(xp_array):
            await ctx.send("Tidak ada history transaksi xp dalam server *{}*.".format(self.suppressed(ctx.guild, ctx.guild.name)))
            return
        ...
        title = "History transaksi XP server {}".format(ctx.guild.name)
        ...
        if page_total > 1:
            help_embed.title = title + " (Halaman {:,} dari {:,})".format(page_count, page_total)
        for i in range(len(xp_array)):
            ...
            f_name = "{} -- ke --> {}".format(from_user, to_user)
            ...
            if len(help_embed.fields) >= 25:
                if page_total == page_count:
                    help_embed.set_footer(text="Requested by " + str(ctx.author.name))
